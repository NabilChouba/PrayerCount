 ...
 *
 * \file    hal_board.c
 * \brief   This file contains functions to handle internal MSP430 settings
 * \version July 2010
 */
...
void halBoardStartXT1(void)
{
    ...
    P7SEL |= 0x03;
    UCSCTL6 |= XCAP_1;          // Set internal cap values to match EZ430 XTAL
                                // 6.5pF each pin, ~ 7pF XTAL req.
 ...
 * \brief   Initializes ACLK output on P2.6
 * \param  void
 * \return void
 */
void halBoardOutputSystemClock(void)
{
    CLK_PORT_DIR |= ACLK_PIN;
    CLK_PORT_SEL |= ACLK_PIN;
}

/**
 * \fn      halBoardStopOutputSystemClock
 * \brief   Stops the output of ACLK on P2.6
 * \param  void
 * \return void
 */
void halBoardStopOutputSystemClock(void)
{
    CLK_PORT_OUT &= ~ACLK_PIN;
    CLK_PORT_DIR |= ACLK_PIN;
    CLK_PORT_SEL &= ~ACLK_PIN;
}
