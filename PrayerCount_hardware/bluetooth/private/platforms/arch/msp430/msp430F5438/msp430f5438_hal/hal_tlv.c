 ...
 * particular 'tag' (in the TLV structure)
 * 
 * NOTE: If 'length' and 'data_address' returned = 0, it implies that there was 
 * no tag match. This condition can be checked for in the main function that
 * calls this Get_TLV_info(..) function and appropriate actions can be taken */

void Get_TLV_info(unsigned char tag, unsigned char *length,
                  unsigned int **data_address)
{
    volatile char *TLV_address = (char *)TLV_START; // TLV Structure Start
                                                    // Address
    ...
    if (*TLV_address == tag)    // Check if Tag match happened..
    {
        ...
        *data_address = (unsigned int *)(TLV_address + 2);  // Return address
                                                            // of first
                                                            // data/value info
                                                            // = Address + 2
    } else                      // If there was no tag match and the end of TLV 
